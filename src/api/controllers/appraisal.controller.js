...
import { ebitdaAvg, ebitdaLastYear, pebitdaLastYear, avgProfit } from '../utils/sharedFunctionsObject'
import { upGreenArrow, downRedArrow, steadyYellowArrow } from '../constants/icons'
// import mailer from '../modules/mailer'
...
const generateAppraisal = async (req, res, next, appraisalId, draft, fromAppraisalList, templatePath, destPdfGenerated, readFile) => {
  try {
    ...
    context.last10BusinessArray = comparableDataSelectedList.map(item => {
      ...
      /* PEBITDA */
      if (appraisal.pricingMethod === 3 || appraisal.pricingMethod === 4 || appraisal.pricingMethod === 7 || appraisal.pricingMethod === 8 || appraisal.pricingMethod === 9 || appraisal.pricingMethod === 10) {
    ...
    const PDF_OPTIONS = {
      ...
      footerTemplate: `
            ...
            <span style="float: left;">Opinion of the market value for ${appraisal.Business.businessName}</span>
    ...
    await page.setContent(template, { timeout: 0, waitUntil: 'networkidle0' })
    ...
    console.log('generateAppraisal -> ', error)
  }
}
...
export const generatePdf = async (req, res, next) => {
  ...
  const { draft, fromAppraisalList } = req.body
...
export const sendEmail = async (req, res, next) => {
  const { appraisalId, businessId, body, subject, to, attachmentName } = req.body
...
export const getEmailTemplateAppraisal = async (req, res, next) => {
  const { templateId, businessId } = req.query
  try {
