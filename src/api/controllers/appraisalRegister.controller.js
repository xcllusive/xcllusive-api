...
export const list = async (req, res, next) => {
  ...
  const where = appraisalRegister ? {
    type: appraisalRegister
  } : null
...
export const create = async (req, res, next) => {
  ...
  try {
    const appraisal = await models.AppraisalRegister.create({
      label,
      type,
      points
    })
    ...
    return res
      ...
      .json({
        data: appraisal,
        message: `Appraisal register ${label} created`
      })
  } catch (error) {
    return next(error)
  }
...
export const update = async (req, res, next) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: `Appraisal register ${label} updated`
    })
  } catch (error) {
    return next(error)
  }
...
export const remove = async (req, res, next) => {
  ...
  try {
    ...
    await models.AppraisalRegister.destroy({
      where: {
        id: appraisalRegisterId
      }
    })
    ...
    return res
      .status(200)
      .json({
        message: `Appraisal register ${appraisalRegisterId} removed with success`
      })
  } catch (error) {
    return next(error)
  }
